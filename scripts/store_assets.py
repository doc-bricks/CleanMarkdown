"""Generate CleanMarkdown Windows Store icons, assets and manifest."""
...
ICON_SOURCE = PROJECT_ROOT / "CleanMarkdown.png"
...
def file_type_association_xml(store_config: dict[str, Any]) -> str:
    file_types = store_config.get("file_types")
    if not file_types or not isinstance(file_types, dict):
        return ""
    name = xml_attr(file_types.get("name", "markdown"))
    display_name = xml_attr(file_types.get("display_name", "Markdown-Dokument"))
    extensions = file_types.get("extensions", [".md", ".markdown", ".mdown", ".mkd"])
    ext_lines = "\n".join(f'            <uap:SupportedFileTypes><uap:FileType>{xml_text(ext)}</uap:FileType></uap:SupportedFileTypes>' for ext in extensions)
    return f"""        <uap:Extension Category="windows.fileTypeAssociation">
...
def render_manifest(store_config: dict[str, Any], exe_name: str | None = None) -> str:
    ...
    identity_name = str(store_config.get("identity_name") or store_config.get("package_id") or "Geiger.CleanMarkdown")
    publisher_id = str(store_config.get("publisher") or store_config.get("publisher_id") or "CN=52596601-BAB4-4F3F-B182-E8F3F273B202")
    version = str(store_config.get("version") or "1.0.2.0")
    description = (
        "Lokaler Markdown-Viewer und -Editor mit Lesemodus, Raw-Editor, "
        "PDF-Export, Mathe-Vorschau und DE/EN-Oberfläche."
    )
    capabilities = capability_lines(store_config.get("capabilities", ["runFullTrust"]))
    fta_xml = file_type_association_xml(store_config)

    extensions_block = ""
    if fta_xml:
        extensions_block = f"""      <uap:Extensions>
{fta_xml}
      </uap:Extensions>"""
