# -*- coding: utf-8 -*-
"""Security and License Contract Test Suite for CleanMarkdown.
...
def test_dependency_vulnerability_floors() -> None:
    ...
    assert any("markdown>=3.4.0" in d for d in deps), (
        "pyproject.toml dependencies must declare markdown>=3.4.0"
    )
    ...
    assert any("ruff>=0.9.0" in d for d in dev_deps), (
        "dev optional-dependencies must declare ruff>=0.9.0"
    )
    assert any("pyinstaller>=6.10.0" in d for d in build_deps), (
        "build optional-dependencies must include pyinstaller>=6.10.0"
    )

    req_text = (PROJECT_ROOT / "requirements.txt").read_text(encoding="utf-8")
    assert "PySide6>=6.10" in req_text, "requirements.txt must declare PySide6>=6.10"
    assert "Markdown>=3.10" in req_text, "requirements.txt must declare Markdown>=3.10"

    req_build = (PROJECT_ROOT / "requirements-build.txt").read_text(encoding="utf-8")
    assert "pyinstaller" in req_build.lower(), (
        "requirements-build.txt must contain pyinstaller"
    )
...
def test_third_party_licenses_inventory_complete() -> None:
    ...
    # Verify license compatibility analysis notes
    assert "LGPL-3.0-only" in text
    assert "PyInstaller Special Exception" in text or "Special Exception" in text
...
def test_no_hardcoded_user_paths_or_dev_secrets() -> None:
    ...
    py_files = [
        p for p in (list(PROJECT_ROOT.glob("*.py")) + list((PROJECT_ROOT / "tests").glob("*.py")))
...
def test_zero_egress_and_offline_invariants() -> None:
    ...
    production_files = [
        ...
        PROJECT_ROOT / "translator.py",
        PROJECT_ROOT / "manage_translations.py",
        PROJECT_ROOT / "generate_store_screenshots.py",
    ]
...
def test_security_policy_bilingual_and_sla() -> None:
    ...
    assert "https://github.com/doc-bricks/CleanMarkdown/security/advisories" in text
