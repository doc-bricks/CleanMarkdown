...
- **Bilder im Lesemodus wirkten wie Hintergrundelemente statt echter Bloecke
  ...
  Nicht-Überlappung, Skalierung, relative Pfade und beide Themes ab.

### Removed
- **Veraltete Web-Companion/PWA-Doku entfernt (T-20260821-242217156):** Die
  Web-Companion/PWA-Oberfläche wurde bereits mit Commit `8cd31be` vollständig
  entfernt (`web_companion/` existiert nicht mehr im getrackten Baum). Ein
  späterer README-Umbau (`d39224a`) hatte "Web Companion (PWA)" versehentlich
  wieder als aktive Produktfläche in die Vergleichstabelle und das
  Architektur-Diagramm von `README.md`/`README_DE.md` eingefügt; die
  ...
  (Zweck-, Status- und Bundleformat-Bulletpoints) auf den tatsächlichen
  Ist-Stand korrigiert, ohne die Oberfläche als künftig geplant
  umzudeuten. Die Flutter-DE/EN-Parität war bereits gegeben und blieb
  unverändert.
